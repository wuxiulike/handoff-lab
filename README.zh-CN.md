...
- `/qa-viewer` 内置本地授权入口：询问、自动、YOLO 完全信任。后端仍兼容旧的 `deny` API 模式，但它不再作为主界面按钮展示。
...
- `VISION_PROVIDER`、`VISION_BASE_URL`、`VISION_MODEL`、`VISION_API_KEY`：可选视觉 QA 模型配置。默认视觉模型配置为 `VISION_BASE_URL=https://api.xiaomimimo.com/v1`、`VISION_MODEL=mimo-v2.5`。
...
`/qa-viewer` 也是开源版的配置入口，包含：

- CLI 授权：询问、自动、YOLO 完全信任。
- 模型配置：DeepSeek/Reasonix worker、Codex CLI、视觉模型。
- 连接测试：DeepSeek、Codex、视觉模型。

当 delegation skill 在询问模式下触发授权拦截时，`/qa-viewer` 会弹出待授权窗口；用户确认后，skill 会继续等待并自动完成启动，不需要在 Codex 对话里手工重试。
